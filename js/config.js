...
function configurarBackupAutomatico(horario) {
    ...
    localStorage.setItem('backup_automatico_horario', horario);
    
    // Verificar se já existe um backup agendado para hoje
    const hoje = new Date().toLocaleDateString();
    const ultimoBackup = localStorage.getItem('ultimo_backup_data');
    
    if (ultimoBackup !== hoje) {
        console.log(`Backup automático configurado para ${horario}`);
    }
}
